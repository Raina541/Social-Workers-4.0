...
export function AuthFlow({ isDarkMode, onAuthComplete }: AuthFlowProps) {
  ...
  const handleOtpVerify = async () => {
    ...
    setErrorMsg(null);
    if (code === '123456') {
      // Instantly accept code 123456 and navigate to signup interests, bypassing the slow backend network call
      setLoading(false);
      navigateTo('SIGNUP_INTERESTS');
    } else {
      setLoading(false);
  ...
  const handleRoleSubmit = async (selectedRole: string) => {
    ...
    // Fire the saveOnboarding request in the background so it doesn't block the UI flow
    authService.saveOnboarding({
      username: formData.username,
      interests: formData.interests,
      role: selectedRole,
    }).catch(err => console.warn('Background saveOnboarding failed:', err));
    ...
    >
      ...
      >
        ...
        {currentStep === 'SIGNUP_CREDENTIALS' && (
          <View style={styles.formContainer}>
            <Text style={[styles.stepIndicator, { color: colors.brandForeground1 }]}>Step 1 of 4</Text>
        ...
        {currentStep === 'SIGNUP_PROFILE' && (
          <View style={styles.formContainer}>
            <Text style={[styles.stepIndicator, { color: colors.brandForeground1 }]}>Step 2 of 4</Text>
        ...
        {currentStep === 'SIGNUP_INTERESTS' && (
          <View style={styles.formContainer}>
            <Text style={[styles.stepIndicator, { color: colors.brandForeground1 }]}>Step 3 of 4</Text>
        ...
        {currentStep === 'SIGNUP_ROLE' && (
          <View style={styles.formContainer}>
            <Text style={[styles.stepIndicator, { color: colors.brandForeground1 }]}>Step 4 of 4</Text>
