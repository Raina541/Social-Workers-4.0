...
import {
  MOCK_OPPORTUNITIES,
  Opportunity as FeedOpportunity,
  ClippedHeroImage,
  FloatingBackground,
  MockMapView,
  generateWindingPath,
...
import { Personalization, CauseType, Opportunity, Idea, NotificationItem, MOCK_FRIENDS, Friend, DOMAIN_TO_CAUSES_MAP } from '../services/personalization';
...
const BOOKMARKED_DOMAIN_TO_FEED_DOMAIN: Record<string, string[]> = {
...
const mapFeedOppToPersonalizationOpp = (feedOpp: FeedOpportunity): Opportunity => {
  const durationStr = feedOpp.durationAbbreviation.toLowerCase();
  let durationHrs = 1.0;
  if (durationStr.includes('h')) {
    durationHrs = parseFloat(durationStr.replace('h', '')) || 1.0;
  } else if (durationStr.includes('m')) {
    durationHrs = (parseFloat(durationStr.replace('m', '')) || 30) / 60;
  }
  
  let distanceKm = 5.0;
  if (feedOpp.location.toLowerCase().includes('morar')) distanceKm = 6.5;
  else if (feedOpp.location.toLowerCase().includes('center')) distanceKm = 3.2;
  
  return {
    id: feedOpp.id,
    title: feedOpp.title,
    cause: 'Education',
    isRemote: feedOpp.timeCommitment.toLowerCase().includes('remote') || feedOpp.location.toLowerCase().includes('remote'),
    distanceKm: distanceKm,
    locationName: feedOpp.location,
...
interface HomeProps {
  ...
  onSelectFeedMode?: (mode: 'In-person' | 'Online' | 'Micro volunteering', timeFilter?: number, domainId?: string) => void;
  activeTab?: number;
...
export const Home: React.FC<HomeProps> = ({
  ...
  const [selectedOnlineOpportunity, setSelectedOnlineOpportunity] = useState<FeedOpportunity | null>(null);
  const [detailContentHeight, setDetailContentHeight] = useState(800);
  const [cityName, setCityName] = useState<string>('Gwalior');
    ...
    const fetchLocation = async () => {
      try {
        ...
        if (status !== 'granted') {
          ...
          if (active) setIsLocationLoading(false);
          return;
        }
  ...
  const handleMicroOppPress = (opp: FeedOpportunity) => {
    if (onSelectFeedMode) {
      onSelectFeedMode('Micro volunteering', undefined, opp.domainId);
    }
    if (onNavigateToTab) {
      onNavigateToTab(1);
    }
  ...
  const loadPersonalizedData = () => {
    ...
    // 2. Fetch opportunities from the Feed data source
    const inPersonFeedOpps = MOCK_OPPORTUNITIES['In-person'] || [];
    const onlineFeedOpps = MOCK_OPPORTUNITIES['Online'] || [];

    // Check bookmarked domains
    const bookmarkedDomains = Personalization.getBookmarkedDomains();
    const allowedFeedDomains = new Set<string>();
    if (bookmarkedDomains && bookmarkedDomains.length > 0) {
      bookmarkedDomains.forEach(domain => {
        const feedDomains = BOOKMARKED_DOMAIN_TO_FEED_DOMAIN[domain] || [];
        feedDomains.forEach(fd => allowedFeedDomains.add(fd));
      });
    }

    const shouldFilter = bookmarkedDomains && bookmarkedDomains.length > 0 && allowedFeedDomains.size > 0;

    const filteredLocalFeed = shouldFilter
      ? inPersonFeedOpps.filter(opp => allowedFeedDomains.has(opp.domainId))
      : inPersonFeedOpps;

    const filteredRemoteFeed = shouldFilter
      ? onlineFeedOpps.filter(opp => allowedFeedDomains.has(opp.domainId))
      : onlineFeedOpps;

    // Map to personalization Opportunity type
    const localOppsMapped = filteredLocalFeed.map(mapFeedOppToPersonalizationOpp);
    const remoteOppsMapped = filteredRemoteFeed.map(mapFeedOppToPersonalizationOpp);

    // 3. Filter local opportunities candidates matching dynamic cityName
    const candidates = localOppsMapped.filter(opp => {
      const targetCity = (cityName || 'Gwalior').toLowerCase();
      return (
        (opp.organizationName || '').toLowerCase().includes(targetCity) ||
        opp.locationName.toLowerCase().includes(targetCity) ||
        opp.title.toLowerCase().includes(targetCity)
      );
    });
    ...
    if (finalOpps.length < 5) {
      const matchedRemote = localOppsMapped.filter(o => o.isRemote);
    ...
    finalOpps.sort((a, b) => {
      ...
      return localOppsMapped.indexOf(a) - localOppsMapped.indexOf(b);
    });

    setLocalOpportunities(finalOpps);

    // 4. Remote opportunities
    setRemoteOpportunities(remoteOppsMapped);
  };

  useEffect(() => {
    loadPersonalizedData();
  }, [pinnedCause, cityName, activeTab]);
  ...
  const getStoryImpactText = (story: Story, cause: CauseType) => {
    ...
    const count = story.contributorsCount || 0;
    return `${count * 5 || 15} helped • ${count || 4} volunteers`;
  };
  ...
  const findFeedOppById = (id: string): FeedOpportunity | undefined => {
    const categories: ('In-person' | 'Online' | 'Micro volunteering')[] = ['In-person', 'Online', 'Micro volunteering'];
    for (const cat of categories) {
      const list = MOCK_OPPORTUNITIES[cat] || [];
      const found = list.find((o: FeedOpportunity) => o.id === id);
  ...
  const handleOpportunityPress = (opp: Opportunity) => {
    const originalFeedOpp = findFeedOppById(opp.id);
    if (originalFeedOpp && onSelectOpportunity) {
      onSelectOpportunity(originalFeedOpp);
      if (onNavigateToTab) {
        onNavigateToTab(1); // Navigates to Feed screen
      }
    } else {
      // Fallback
  ...
  const mainTranslateX = microVolunteeringAnim.interpolate({
    inputRange: [0, 1],
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.neutralBackground2 }]}>
      ...
      <ScrollView ref={mainScrollViewRef} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            return (
              <View>
                ...
                <View style={styles.opportunitiesStack}>
                  {localOpportunities.slice(0, 2).map(opp => (
                    <OpportunityCard
                      key={opp.id}
                      opportunity={opp}
            ...
            return (
              <View style={{ padding: Spacing.m, alignItems: 'center', marginVertical: Spacing.xs }}>
                <Text style={{ color: themeColors.neutralForeground3, fontSize: 13 }}>
                  No local opportunities matching {cityName} radius.
                </Text>
              </View>
            );
          }
        ...
        <View style={styles.opportunitiesStack}>
          {(() => {
            if (remoteOpportunities.length > 0) {
              return remoteOpportunities.slice(0, 2).map(opp => {
                const originalFeedOpp = findFeedOppById(opp.id);
                return (
                  <OpportunityCard
                    key={opp.id}
                    ...
                    onPress={() => {
                      if (originalFeedOpp) {
                        setSelectedOnlineOpportunity(originalFeedOpp);
                      } else {
                        handleOpportunityPress(opp);
                      }
                    }}
                    isDarkMode={isDarkMode}
                  />
              ...
              return (
                <View style={{ padding: Spacing.m, alignItems: 'center', marginVertical: Spacing.xs }}>
                  <Text style={{ color: themeColors.neutralForeground3, fontSize: 13 }}>
                    No remote opportunities matching bookmarked domains.
                  </Text>
                </View>
              );
            }
              ...
              <View style={[styles.storyGradient, { flex: 1, zIndex: 10 }]}>
                ...
                <View style={[styles.storyContentArea, { zIndex: 12, paddingBottom: insets.bottom > 0 ? insets.bottom + 16 : 24 }]}>
                  ...
                  <View style={styles.storySocialRow}>
                    {(activeStories[storyIndex]?.contributorsCount ?? 0) > 0 && (
                      <View style={styles.storyAvatarsStack}>
                        {(activeStories[storyIndex]?.contributorsAvatars ?? []).slice(0, 3).map((url: string, i: number) => (
                          <Image
                            key={url}
                            source={{ uri: url }}
      ...
      {/* Online/Remote Detailed View Modal */}
      {selectedOnlineOpportunity && (
        <Modal
          visible={selectedOnlineOpportunity !== null}
          animationType="slide"
          presentationStyle="fullScreen"
          onRequestClose={() => setSelectedOnlineOpportunity(null)}
        >
          <OpportunityDetail
            ...
            onViewNgo={(ngoName) => {
              setSelectedOnlineOpportunity(null);
              if (onViewNgo) {
                onViewNgo(ngoName);
              }
            }}
